...
export default function HouseholdClient({ userId, household, members }: Props) {
  ...
  async function enrichAllItems() {
    ...
    const { data: items } = await supabase
      .from('items')
      .select('id, name, category, default_unit, emoji, estimated_price, canonical_name')
      .eq('household_id', household.id)
      .eq('active', true)
      .or('estimated_price.is.null,canonical_name.is.null')
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // One retry — a transient API error shouldn't permanently strand an
      // item without a canonical name until someone notices and re-runs
      // this whole batch by hand.
      const result = await refetchEnrichment(enrichmentParams) ?? await refetchEnrichment(enrichmentParams)
      if (result) {
        // Only backfill price + canonical name + (if still default) emoji —
        // an existing item's category/unit/location were already set
        // deliberately when it was added, so a bulk pass shouldn't silently
        // change them.
        const updates: { estimated_price?: number; emoji?: string; canonical_name?: string } = {}
        if (item.estimated_price === null) updates.estimated_price = result.estimated_price
        if (!item.canonical_name && result.canonical_name) updates.canonical_name = result.canonical_name
        if (item.emoji === '📦' && result.emoji) updates.emoji = result.emoji
        if (Object.keys(updates).length > 0) await supabase.from('items').update(updates).eq('id', item.id)
      }
  ...
  return (
    <AppBackground>
      <PageHeader title="Household" backHref="/settings" />
      ...
      <div style={{ padding: '20px 20px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        >
          ...
          />
          {enrichingAll
            ? `Enriching items… ${enrichProgress?.done ?? 0}/${enrichProgress?.total ?? 0}`
            : 'Estimate prices for existing items'}
        </button>
        <p className="text-11" style={{ color: 'var(--muted)', marginTop: -8 }}>
          Fills in price estimates and ingredient-matching data for items missing them — doesn&apos;t change category, unit, or location.
        </p>
